...
## Levels 1 and 2 are the two drawings; after that layouts are random, and every random
## layout is checked with the enemy's own solver so a shot always exists from both sides.
func _build_layout(level: int) -> Dictionary:
	_clear_planets()
	if level == 1:
		_add_planet(Vector2(330, 185), 95.0)
		return {"pp": 0, "pa": deg_to_rad(140.0), "ep": 0, "ea": deg_to_rad(-25.0)}
	if level == 2:
		_add_planet(Vector2(100, 285), 66.0)
		_add_planet(Vector2(535, 128), 66.0)
		_add_planet(Vector2(300, 175), 26.0)
		return {"pp": 0, "pa": deg_to_rad(-10.0), "ep": 1, "ea": deg_to_rad(190.0)}

	var n_big := 2 + mini(2, (level - 3) / 2)
	var n_moon := mini(3, (level - 1) / 2)
	var best: Dictionary = {}
	var best_miss := INF
	for attempt in 8:
		_clear_planets()
		var ok := true
		for i in n_big:
			if not _place_random(randf_range(42.0, 78.0)):
				ok = false
		for i in n_moon:
			if not _place_random(randf_range(16.0, 28.0)):
				ok = false
		if not ok:
			continue
		# the big planets come first in the list; you get the leftmost, the enemy the rightmost
		var left := 0
		var right := 0
		for i in n_big:
			if _planets[i]["pos"].x < _planets[left]["pos"].x:
				left = i
			if _planets[i]["pos"].x > _planets[right]["pos"].x:
				right = i
		if left == right:
			continue
		var pa := _surface_angle(left, right)
		var ea := _surface_angle(right, left)
		var p_pos := _tank_pos(left, pa)
		var e_pos := _tank_pos(right, ea)
...
func _place_random(r: float) -> bool:
	...
	for i in 40:
		...
		var clear := true
		for pl in _planets:
			if p.distance_to(pl["pos"]) < r + pl["r"] + 46.0:
				clear = false
				break
		if clear:
			_add_planet(p, r)
...
func _surface_angle(on: int, other: int) -> float:
	...
	for i in 12:
		...
		if play_area.grow(-14.0).has_point(_tank_pos(on, a)):
			return a
	return base
...
func _move_shells(delta: float) -> void:
	...
	for s in _shells:
		...
		if mine:
			_last_trail = path
		s.queue_free()
		if hit.is_empty() or hit["what"] == "space":
...
func _draw() -> void:
	...
	_text(f, 320, 22, "level %d" % _level, 13, Palette.col("ink"))
	if _prompt != "":
...
func _draw_preview() -> void:
	...
	for i in PREVIEW_STEPS:
		...
		if i % 5 == 4:
			...
			draw_circle(p, 1.6, Color(c.r, c.g, c.b, 0.25 + 0.45 * k))
	# the aim point itself, a ring so the shell stays visible under it
	draw_arc(_player.position + _aim, 7.0, 0.0, TAU, 24, Color(c.r, c.g, c.b, 0.6), 1.5, true)
...
func _draw_clock() -> void:
	...
	draw_arc(_player.position, TANK_R + 8.0, -PI * 0.5, -PI * 0.5 + TAU * frac, 32, c, 2.0, true)

func _text(f: Font, cx: float, y: float, msg: String, size: int, col: Color) -> void:
	draw_string(f, Vector2(cx - 220, y), msg, HORIZONTAL_ALIGNMENT_CENTER, 440, size, col)
